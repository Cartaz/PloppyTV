...
normalize = Path('src/lib/normalize.ts')
text = normalize.read_text()
old_doc = ''' * BUG-02-07 (FIXED): chiavi stagione validate con safeId (regex ^-?\\d+$),
 * quindi " 1 ", "1.5", "0x10", "1e2" sono rifiutate.
'''
new_doc = ''' * Le chiavi stagione persistite devono usare la rappresentazione decimale
...
if old_doc not in text:
    raise SystemExit('normalize documentation anchor not found')
text = text.replace(old_doc, new_doc, 1)
old_code = '''      // BUG-02-07: valida la chiave con safeId (regex ^-?\\d+$); " 1 ", "1.5",
      // "0x10", "1e2" sono rifiutate.
      const seasonKey = safeId(k);
      if (!seasonKey) continue;
'''
new_code = '''      const seasonKey = safeId(k);
...
if old_code not in text:
    raise SystemExit('season key code anchor not found')
normalize.write_text(text.replace(old_code, new_code, 1))

edge = Path('tests/normalize.edgecases.test.ts')
edge.write_text(r'''import { describe, expect, it } from 'vitest';
...
probe = Path('tests/probe_normalize.test.ts')
if not probe.exists():
    raise SystemExit('probe_normalize.test.ts not found')
probe.unlink()

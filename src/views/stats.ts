...
function renderStatsContent(main: HTMLElement, stats: StatsResult): void {
  ...
  html +=
    ...
    '<div class="stat-card"><div class="stat-value">' + escapeHtml(stats.timeLabel) + '</div><div class="stat-label">Tempo totale</div></div>' +
    ...
    '<div class="stat-card"><div class="stat-value">' + totalProgress + '%</div><div class="stat-label">Progresso totale</div></div>' +
    '</div>';

  // Barra di progresso totale: 100% → verde (commit ba32535)
  html += '<div class="section"><div style="height:12px;background:var(--bg-card);border-radius:6px;overflow:hidden;">' +
    '<div style="height:100%;width:' + totalProgress + '%;background:' + (totalProgressComplete ? 'var(--success, #4ade80)' : 'var(--accent)') + ';transition:width .3s;"></div></div></div>';
  ...
  if (stats.topShows.length === 0) {
    ...
    for (const item of stats.topShows) {
      ...
      html +=
        ...
        '<div class="episode-meta">' + item.watched + '/' + item.totalEpisodes + ' episodi • ' + pct + '%</div></div></div>';
    }
    html += '</div>';
  }
  html += '</div>';

...
//
// BUG-08-01 (High): `worker.onerror` now disables the worker
// (`_workerSupported = false; _worker = null;`) so subsequent requests fall
// back immediately instead of re-posting to a dead worker forever.
//
// BUG-08-02 (Medium): the timeout was a hard-coded 500ms — too short for big
// libraries, causing spurious double-compute (worker + main-thread fallback).
// Raised to `WORKER_TIMEOUT_MS` (3000ms).
//
// BUG-08-03 (Low) + BUG-16-03 (Medium): the fallback `computeStatsFallback` /
// `computeCalendarFallback` were copy-pastes of the worker code (drift risk)
// and the calendar fallback lacked the worker's `safeOffset` guard. Both now
// delegate to the shared pure functions in `./compute.ts` — single source of
// truth, identical behavior in both paths.
...
/** Timeout before falling back to main-thread compute (BUG-08-02). */
...
function getWorker(): Worker | null {
  ...
  try {
    ...
    // BUG-08-01: disable the worker so subsequent requests skip it and fall
    // back immediately, instead of re-posting to a dead worker → 500ms wait
    // → main-thread fallback on EVERY request forever.
    _worker.onerror = (e) => {
